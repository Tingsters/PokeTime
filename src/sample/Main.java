...
public class Main extends Application {
    ...
    private static Timeline battle = new Timeline(new KeyFrame(Duration.seconds(5), actionEvent -> {
        ...
        shrink.setToX(1);
        shrink.setToY(1);
        SequentialTransition attackTransition = new SequentialTransition(battleSceneController.pokemonPicture, grow, shrink);
    ...
    public static void attackImpl(int damage) {
        if (!inBattle || gameover) return;
        TranslateTransition right = new TranslateTransition(Duration.seconds(.2), battleSceneController.trainerPokemon);
        right.setToX(10);
    ...
    public static void earthquake() {
        ...
        quakeTimeline.getKeyFrames().add(new KeyFrame(Duration.seconds(10), new KeyValue(spriteGroup.translateXProperty(), 0)));
